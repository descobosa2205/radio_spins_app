...
| `check_camerinos.py` | la hoja de ruta en CAMERINOS (la pantalla de los Echo Show, `/camerinos`): elegir cuál y qué hoja se ve (solo una en toda la casa), que enseña solo los horarios de esa hoja sin filtrar teléfonos, notas ni habitaciones, la versión del sondeo, los permisos y el botón del panel; los AVISOS a las pantallas (mandar, los rápidos, visto en x de y, retirar, caducar, la voz); las HOJAS DE RUTA con nombre e icono; y el control de camerinos (`/controlcamerinos`, en abierto y acotado por su llave) | al tocar `/camerinos`, `/controlcamerinos`, el botón «Camerinos» del panel, `_camerinos_item`, los avisos o el catálogo de hojas de ruta |
